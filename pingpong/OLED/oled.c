...
volatile uint8_t active_buf = 0; // 0 or 1: where drawing functions write (back buffer)
...
void start_timer_transfer(void) {
	...
	if (ocr > 255) {
		prescaler = 1024;
		ocr = (F_CPU / (prescaler * interrupts_per_sec)) - 1;
	}
	
	TCCR3B = (1 << WGM32); // CTC
	OCR3A = ocr;
	switch (prescaler) {
		case 64: TCCR3B = (1 << CS22); break;
		case 1024: TCCR3B = (1 << CS22) | (1 << CS21) | (1 << CS20); break;
	}
